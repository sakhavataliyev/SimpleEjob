...
[![Build Status img](https://travis-ci.org/fengyuanchen/cropper.svg)](https://travis-ci.org/fengyuanchen/cropper)
...
**Note:** Don't trust this all the time as some JPG img have incorrect (not standard) Orientation values.
...
- **onlyColorChanged** (optional):
  - Type: `Boolean`
  - If only change the color, not the size, then the cropper only need to change the srcs of all related img, not need to rebuild the cropper. This can be used for applying filters.
