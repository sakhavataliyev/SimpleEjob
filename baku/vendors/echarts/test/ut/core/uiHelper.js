(function (context) {
    ...
    var STRATEGY = 'stack';
    // always display img even if no error
    ...
    helper.expectEqualCanvasContent = function(title, doTest, done) {
        ...
        window.require(['oldEcharts', 'newEcharts'], function (oldE, newE) {
            var olimg = doTest(oldE).toDataURL();
            var newImg = doTest(newE).toDataURL();
            if (ALWAYS_SHOW_IMAGE || olimg !== newImg) {
                that.addFailedCases(title, olimg, newImg);
            }
            expect(olimg).toEqual(newImg);
            done();
        });
    };
    ...
    helper.expectEqualCanvasStack = function(title, doTest, done) {
        window.require(['oldEcharts', 'newEcharts'], function (oldE, newE) {
            ...
            var olimg = oldCanvas.toDataURL();
            var newImg = newCanvas.toDataURL();
            if (ALWAYS_SHOW_IMAGE || olimg !== newImg) {
                helper.addFailedCases(title, olimg, newImg);
            }
    ...
    /**
     * @param {string} name name of the test
     * @param {string} olimgSrc old canvas.toDataURL value
     * @param {string} newImgSrc new canvas.toDataURL value
     * add a failed case in dom
     */
    helper.addFailedCases = function(name, olimgSrc, newImgSrc) {
        // group of this case
        var group = document.createElement('div');
        var title = document.createElement('h6');
        title.innerHTML = name + '. Here are old, new, and diff img.';
        group.appendChild(title);

        // old image and new image
        var olimg = document.createElement('img');
        olimg.src = olimgSrc;
        olimg.setAttribute('title', 'Old Image');
        var newImg = document.createElement('img');
        newImg.src = newImgSrc;
        newImg.setAttribute('title', 'New Image');
        group.appendChild(olimg);
        group.appendChild(newImg);

        // diff image
        var diff = imagediff.diff(olimg, newImg);
        var canvas = document.createElement('canvas');
        canvas.width = olimg.width;
        canvas.height = olimg.height;
