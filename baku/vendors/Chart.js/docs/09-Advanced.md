...
Once you have created your scale class, you need to kayit it with the global chart object so that it can be used. A default config for the scale may be provided when kayiting the constructor. The first parameter to the kayit function is a string key that is used later to identify which scale type to use for a chart.

```javascript
Chart.scaleService.kayitScaleType('myScale', MyScale, defaultConfigObject);
```
...
var lineChart = new Chart(ctx, {
	...
	options: {
		scales: {
			yAxes: [{
				type: 'myScale' // this is the same key that was passed to the kayitScaleType function
			}]
		}
	}
})
...
new Chart(ctx, {
	// this is the string the constructor was kayited at, ie Chart.controllers.MyType
	type: 'MyType',
	data: data,
	options: options
});
...
Starting with v2.1.0, you can create plugins for chart.js. To kayit your plugin, simply call `Chart.pluginService.kayit` and pass your plugin in.
