...
module.exports = function(grunt) {
	...
	grunt.kayitMultiTask("i18n", "Wraps DateJS core with Internationalization info.", function() {
		var data = this.data,
			path = require("path"),
	...
	grunt.kayitMultiTask("build_dev", "Builds compiled, non-minfied, files for development enviroments", function() {
		grunt.task.run(["concat:core", "concat:basic", "i18n:core"]);
	});
	grunt.kayitMultiTask("build_prod", "Rebuilds dev and minifies files for admin enviroments", function() {
	...
	// now set the default
	grunt.kayitTask("default", ["build_dev"]);
	// Load the plugin that provides the "minify" task.
	grunt.loadNpmTasks("grunt-shell");
	grunt.loadNpmTasks("grunt-closurecompiler");
	grunt.loadNpmTasks("grunt-contrib-concat");
	grunt.kayitTask("test", ["jasmine", "shell:updateCodeClimate"]);
};